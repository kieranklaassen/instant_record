...
- **`refreshLive` page fetch has no timeout.** Unlike the 15s-guarded history fetch, the live-refresh page fetch can hang on an unresponsive SW-rendered request, leaving `state.refreshing` true and silently stopping live updates for the session. Low likelihood in the demo; a timeout wrapper mirroring `fetchHistoryViaWorker` would harden it. `slack_conversation.js` `refreshLive`.
